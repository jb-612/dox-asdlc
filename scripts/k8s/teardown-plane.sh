...
# teardown-plane.sh - Remove Plane CE deployment from Kubernetes
#
# Usage: ./scripts/k8s/teardown-plane.sh [options]
#
# Options:
...
print_summary() {
    echo ""
    echo "=========================================="
    echo "  Plane CE Teardown Complete"
    echo "=========================================="
    echo ""
...
# Main execution
main() {
    log "Starting Plane CE teardown..."

    check_prerequisites
    show_resources

    # Confirmation
    local confirm_msg="This will uninstall the Plane CE release."
    if [ "$DELETE_DATA" = true ]; then
